...
class Stefan1D_direct(tf.Module):
    def __init__(self, layers_u, layers_s, ics_sampler, Ncs_sampler, res_sampler):
        super(Stefan1D_direct, self).__init__()  # Инициализация базового класса
    ...
    def train_step(self, tf_dict):
        with tf.GradientTape() as tape:
            ...
            self.u_Sbc_pred = self.net_u((self.s_pred - self.mu_x) / self.sigma_x, tf_dict['t_u_tf'])
            self.s_0_pred = self.net_s(tf_dict['t_0_tf'])
            self.u_Nc_pred = self.net_u_x(tf_dict['x_Nc_tf'], tf_dict['t_Nc_tf'])

            self.r_u_pred = self.net_r_u(tf_dict['x_r_tf'], tf_dict['t_r_tf'])
            self.r_Nc_pred = self.net_r_Nc(tf_dict['t_Nc_tf'])
            ...
            self.loss_SNc = tf.reduce_mean(tf.square(self.r_Nc_pred - tf_dict['s_Nc_tf']))
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in range(nIter):
            ...
            tf_dict = {
                ...
                'x_Nc_tf': tf.convert_to_tensor(X_Ncs_batch[:, 0:1], dtype=tf.float32),
...
class Stefan1D_inverse_I:
    def __init__(self, layers_u, ics_sampler, Ncs_sampler, res_sampler):
        ...
        self.x_u_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.t_u_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.u_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)

        self.x_ic_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.t_ic_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.u_ic_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)

        self.x_Nc_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.t_Nc_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.u_Nc_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.s_Nc_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)

        self.x_r_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
        self.t_r_tf = tf.convert_to_tensor([[0.0]], dtype=tf.float32)
...
class Stefan1D_inverse_II:
    ...
    def net_s_t(self, t):
       with tf.GradientTape() as tape:
          tape.watch(t)
          s = self.net_s(t)
       s_t = tape.gradient(s, t) / self.sigma_t
       return s_t
